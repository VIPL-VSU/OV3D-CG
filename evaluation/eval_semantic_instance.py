...
from scannet_constants import SCANNET_COLOR_MAP_20, CLASS_LABELS_20, VALID_CLASS_IDS_20, SCANNET_COLOR_MAP_200, CLASS_LABELS_200, VALID_CLASS_IDS_200
...
def compute_iou_matrix(masks):
    """计算所有 mask 之间的 IoU 矩阵（批量计算）"""
    masks = masks.astype(bool)
    M = masks.shape[0]

    # 计算交集（按位 AND），形状 (M, M)
    intersection = np.dot(masks, masks.T)

    # 计算并集（所有像素和 - 交集），形状 (M, M)
    area = masks.sum(axis=1, keepdims=True)
    union = area + area.T - intersection

    # 避免除以 0
    iou_matrix = np.divide(intersection, union, where=(union > 0))

    return iou_matrix


def fast_nms_binary_masks(masks, iou_threshold=0.5):
    """
    用 NumPy 加速 NMS 删除 IoU 过高的 mask

    masks: (M, N) 的二进制 NumPy 数组
    iou_threshold: IoU 阈值

    返回:
    - `keep_indices`: 经过 NMS 筛选后保留的 mask 索引
    """
    M = masks.shape[0]
    if M == 0:
        return []

    # 计算所有 mask 之间的 IoU 矩阵
    iou_matrix = compute_iou_matrix(masks)

    # 按照 mask 的顺序处理
    keep = []
    suppressed = np.zeros(M, dtype=bool)  # 记录哪些 mask 被去除
    ...
    for i in range(M):
        ...
        # 找到 IoU 超过阈值的 mask，并标记为去除
        suppressed[iou_matrix[i] > iou_threshold] = True
...
def evaluate_matches(matches):
    ...
    for scene_name in gt_category_names.keys():
        pcdc_scores.append(np.asarray(
            [len(matched_predictions_category_names[d_inf][oid][scene_name]) / len(gt_category_names[scene_name]) for
             oid in matched_predictions_category_names[d_inf].keys()]))
...
def print_results_ap_ar_rc_pcdc(avgs, ar_avgs, rc_avgs, pcdc_avgs,
    ...
    # 使用 redirect_stdout 将所有的print内容保存到文件
...
def write_result_file(avgs, filename):
    ...
    with open(filename, 'w') as f:
        ...
        f.write(_SPLITTER.join(['head', 'all', str(avgs["all_ap"]), str(avgs["all_ap_50%"]), str(avgs["all_ap_25%"])]))
        f.write(_SPLITTER.join(['all_ap', 'all', str(avgs["all_ap"]), str(avgs["all_ap_50%"]), str(avgs["all_ap_25%"])]))
...
def evaluate(preds: dict, gt_path: str, output_file: str, dataset: str = "scannet"):
    ...
    if dataset == "scannet200":
        ...
        HEAD_CATS_SCANNET_200 = set(
            ['tv stand', 'curtain', 'blinds', 'shower curtain', 'bookshelf', 'tv', 'kitchen cabinet',
    ...
    # print
    # print_results(avgs)
    print_results_ap_ar_rc_pcdc(avgs, ar_avgs, rc_avgs, pcdc_avgs, output_file=output_file)
...
def main():
    # #eval mask3d
    with open('val_scenes_scannet200.txt') as f:
        scene_ids = f.readlines()
    mask_scene_path1 = "../sai3d_masks/"
    mask_scene_path2 = "../mask3d_masks/"
    preds = {}
    for scene_id in tqdm(scene_ids):
        scene_id = scene_id.strip()
        pred_mask1 = torch.load(f"{mask_scene_path1}/{scene_id}_masks.pt")
        pred_mask2 = torch.load(f"{mask_scene_path2}/{scene_id}_masks.pt")

        label = []
        mask_dir1 = "../sai3d_masks_2d/{}".format(scene_id)
        mask_dir2 = "../mask3d_masks_2d/{}".format(scene_id)
        with open(os.path.join(mask_dir1, "sam_mask_cot.txt"), "r") as f:
            label_array = [line.strip() for line in f]
        for name in label_array:
            if name in CLASS_LABELS_200:
                x = CLASS_LABELS_200.index(name)
                label.append(VALID_CLASS_IDS_200[x])
            else:
                label.append(-1)
        with open(os.path.join(mask_dir2, "sam_mask_cot.txt"), "r") as f:
            label_array = [line.strip() for line in f]
        for name in label_array:
            if name in CLASS_LABELS_200:
                x = CLASS_LABELS_200.index(name)
        ...
        pred_classes = np.array(label)
        pred_scores = np.ones(pred_classes.shape)
        pred_mask = np.hstack([pred_mask1, pred_mask2])
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    evaluate(preds, gt_dir, output_file= os.path.join(out_dir, 'scannet200_res.txt'), dataset='scannet200')
